...
double findVariance_Parallel(float *globalData, int globalSize, int localSize, int rank, int numProcs, float mean)
{
    double variance = 0.0;
    MPI_Bcast(&localSize, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    // Calculate using E[X²] - (E[X])² approach which is more numerically stable
    float localSum = 0.0f;
    float localSumSquares = 0.0f;
    for (int i = 0; i < localSize; i++)
    {
        localSum += localData[i];
        localSumSquares += localData[i] * localData[i];
    }

    float globalSum = 0.0f;
    float globalSumSquares = 0.0f;
    MPI_Reduce(&localSum, &globalSum, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&localSumSquares, &globalSumSquares, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);

    if (rank == 0)
    {
        float globalMean = globalSum / globalSize;
        variance = (globalSumSquares / globalSize) - (globalMean * globalMean);
    }
...
main(int argc, char **argv)
{
    ...
    //
    double variance = 0.0f;      // Your calculated variance should be placed in this variable.
